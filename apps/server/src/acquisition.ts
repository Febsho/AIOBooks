...
import { type AcquisitionProfile, type MatchResult, type NormalizedRelease } from "@aiobooks/core";
...
export interface AcquisitionQueue {
  ...
  enqueueRemoteOutput(remoteOutputId: string): Promise<void>;
}

export function compatibilityKey(profile: AcquisitionProfile): string {
...
export async function loadProfile(sql: Database, userId: string, isAdmin: boolean, profileId: string): Promise<AcquisitionProfile | null> {
...
export async function storedReleases(sql: Database, requestId: string, userId: string, isAdmin: boolean) {
...
type SelectReleaseResult =
  | { downloadJobId: string; requestId: string }
  | { conflict: string }
  | { noDownloader: true }
  | null;
...
export async function selectRelease(
  ...
  return sql.begin(async (transaction) => {
    const [row] = await transaction<{
      ...
      SELECT req.id AS request_id, aj.id AS acquisition_job_id, aj.state AS acquisition_state, rel.id AS release_id, rel.normalized_data
      FROM requests req JOIN acquisition_job_requests ajr ON ajr.request_id = req.id JOIN acquisition_jobs aj ON aj.id = ajr.acquisition_job_id
      JOIN release_searches rs ON rs.acquisition_job_id = aj.id JOIN releases rel ON rel.release_search_id = rs.id
      WHERE req.id = ${requestId} AND rel.id = ${releaseId} AND (${user.role === "ADMIN"} OR req.owner_user_id = ${user.id})
      FOR UPDATE OF aj
    `;
    if (!row) return null;
    ...
    const [connection] = await transaction<{ id: string }[]>`
      SELECT c.id FROM connections c LEFT JOIN connection_permissions cp ON cp.connection_id = c.id AND cp.user_id = ${user.id}
    ...
    const [download] = await transaction<{ id: string }[]>`
      ...
      ON CONFLICT (acquisition_job_id) DO UPDATE SET updated_at = now() RETURNING id
    `;
    await transaction`UPDATE acquisition_jobs SET selected_release_id = ${row.release_id}, selected_release = ${transaction.json(JSON.parse(JSON.stringify(row.normalized_data)))}, state = 'QUEUED', updated_at = now() WHERE id = ${row.acquisition_job_id}`;
    await transaction`
    ...
    `;
    await transaction`UPDATE requests SET state = 'QUEUED', updated_at = now() WHERE id IN (SELECT request_id FROM acquisition_job_requests WHERE acquisition_job_id = ${row.acquisition_job_id}) AND state = 'MATCHED'`;
    return { downloadJobId: download!.id, requestId: row.request_id };
  });
}
...
export function registerAcquisitionRoutes(
  ...
  app.post("/api/requests/:id/releases/:releaseId/select", write, async (request, reply) => {
    ...
    const user = request.authUser!;
    const selected = await selectRelease(sql, user, params.data.id, params.data.releaseId);
  ...
  app.post("/api/books/:id/request", write, async (request, reply) => {
    ...
    const searchResult = await executeAcquisitionSearch({ sql, cipher: credentialCipher, jobId: attached.jobId, correlationId: request.id, user, work, profile });
    const nextState = searchResult.state;
    const persisted = await storedReleases(sql, attached.requestId, user.id, user.role === "ADMIN");
    let responseState: string = nextState;
    if (input.data.automatic && queue && persisted[0]) {
      const selected = await selectRelease(sql, user, attached.requestId, persisted[0].id);
      if (selected && "downloadJobId" in selected) {
        responseState = "QUEUED";
        try { await queue.enqueueDownload(selected.downloadJobId); }
        catch (error) { request.log.warn({ err: error, downloadJobId: selected.downloadJobId }, "download dispatch failed; database recovery will retry"); }
      }
    }
    return reply.code(201).send({ requestId: attached.requestId, state: responseState, releases: persisted.map((row) => ({ id: row.id, release: row.normalized_data, match: row.match_data, score: row.rank_score, scoreReasons: row.match_data.scoreReasons ?? [] })), diagnostics: searchResult.diagnostics });
  });
